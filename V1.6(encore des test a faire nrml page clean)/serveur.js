...
function afficher_donnees() {
    ...
    fetch("./serveur.json?t=" + Date.now())  // ✅ Contournement du cache avec ./ au début
        .then(function(response) {
            if (!response.ok) throw new Error("Erreur lors du chargement des données");
            return response.json();
        })
        .then(function(data) {
            const utilisateur = document.getElementById("photo_p").dataset.username;
            ...
            if (data.S1) {
                ...
                if (data.S1.launch === true && joueurs1.includes(utilisateur)) {
            ...
            if (data.S2) {
                ...
                if (data.S2.launch === true && joueurs2.includes(utilisateur)) {
                    window.location.href = "Jeu.html";
                    return;
                }
